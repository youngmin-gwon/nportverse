// Original Link: https://www.shadertoy.com/view/XtKSWc
...
vec3 texpent(in vec2 p, in float idx) {
    ...
    vec2 q = abs(p);
    float rz = sin(clamp(max(max(q.x * 1.176 - p.y * 0.385, q.x * 0.727 + p.y), -p.y * 1.237) * 33., 0., 25.)) * siz + siz;
    vec3 col = (sin(vec3(1, 1.5, 5) * idx) + 2.) * (rz + 0.25);
    col -= sin(dot(p, p) * 10. + iTime * 5.) * 0.4;
    return col;
}
...
vec3 texcub(in vec2 p, in float idx) {
    ...
    col -= sin(a * 15. + iTime * 11.) * 0.15 - 0.15;
    return col;
}
...
vec3 textri(in vec2 p, in float idx) {
    ...
    col -= sin(p.x * 10. + iTime * 8.) * 0.15 - 0.1;
    return col;
}
...
void main() {
    ...
    mat2 mx = mm2(iTime * 0.15 + um.x * 6.);
    mat2 my = mm2(iTime * 0.18 + um.y * 6.);
    ro.xz *= mx;
    rd.xz *= mx;
    ro.xy *= my;
    rd.xy *= my;

    float sel = mod(floor((iTime + 10.) * 0.2), 4.);
    //sel=0.;
